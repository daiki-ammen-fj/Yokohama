...
DEVICE_CREDENTIALS = {
    ...
    "DUT_ZCU670": {
        "ip": "10.18.180.151",
        "user": "petalinux",
        "password": "petalinux00",
        "sudo_password": "petalinux00",
        "port": "COM4",
        "baudrate": "115200",
    },
}
...
parser.add_argument(
    "-b",
    "--batch",
    help="Batch script path",
    default=os.getenv("BATCH_SCRIPT_PATH", r"C:\Users\labuser\qlight-control\run_qlight_check.bat"),
)
parser.add_argument(
    "-p",
    "--paam",
    help="PAAM script path",
    default=os.getenv("PAAM_SCRIPT_PATH", r"C:\Users\labuser\pybeacon\run_paam_dl_reg.bat"),
)
...
parser.add_argument(
    ...
    choices=["SSH", "Serial"],
    required=True,
    help="Specify the connection type for DUT (SSH or Serial)",
)
        ...
        if dut_credentials:
            if args.connection == "SSH":
                ...
                setup_dut(
                    ...
                    port=dut_credentials["port"],
                    baudrate=dut_credentials["baudrate"],
                )
            else:
                logging.error("Invalid connection type specified.")
        else:
            logging.error("DUT credentials are missing.")
